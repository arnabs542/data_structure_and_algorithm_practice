...
def numUniqueEmails(emails):
    ...
    for email in emails:
        ...
        simplified_local_name = []
        letter_index = 0
        while letter_index < len(local_name):
            if local_name[letter_index] == '+':
                break
            elif local_name[letter_index] != '.':
                simplified_local_name.append(local_name[letter_index])
            letter_index += 1

        simplified_email = ''.join(simplified_local_name) + '@' + domain_name
        if simplified_email not in unique_emails:
