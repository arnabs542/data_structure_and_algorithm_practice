...
line number 2: <noOfNdoes space seprated integers> denoting the values of the nodes. Please make sure there are not leading
 or trailing spaces and between two integers there must be a single space.
line number 3: <noOfNdoes space seprated string> denoting the values of the next right node. If there are no next right
 node then print the string "null" without quote. Please make sure there are not leading or trailing spaces and between
 two integers there must be a single space.
...
Sample Test Case:
Sample Input:
https://i.imgur.com/zRznISe.png

Sample Output:
https://i.imgur.com/ZpgbIQO.png
...
In third level, there are 4 nodes. Second node is next right of first node, third node is next right to second node and
fourth node is next right of third node.
...
class TreeNode():
    def __init__(self, val=None, left_ptr=None, right_ptr=None, nextRight=None):
        self.val = val
...
class BinaryTree():
    ...
    def considerEdge(self, lOrR, parent, child, nodeToId, id, edgeCreateCycle):
        if child != None:
            if child in nodeToId:
                ...
                self.nextRightNodes.append(child.nextRight)
    ...
    def populateRawValuesFromTree(self):
        ...
        if self.root != None:
            ...
            self.nextRightNodes.append(self.root.nextRight)
    ...
    def writeRawValues(self):
        ...
        for i in range(self.noOfNodes):
            if i == self.noOfNodes - 1:
                if self.nextRightNodes[i] == None:
                    print("null")
                else:
                    print(self.nextRightNodes[i].val)
            else:
                if self.nextRightNodes[i] == None:
                    print("null", end=' ')
                else:
                    print(self.nextRightNodes[i].val, end=' ')
...
def main():
    root = readBinaryTree()
    result = populateSiblingPointers(root)
...
In this solution we have followed BFS or level wise traversal approach to populate the next right pointer. We have pushed
 the nodes level wise and processed level by level. Let’s say in a level there are K nodes. In that case, i’th nodes nextRight
 node is (i+1)th node where i < K-1.
...
In this solution we have followed DFS traversal approach to populate the next right pointer. Left child’s nextRight pointer
 is right child. Right child’s nextRight pointer is parent’s nextRight node’s left pointer.
Time complexity:
O(N)
Auxiliary space:
