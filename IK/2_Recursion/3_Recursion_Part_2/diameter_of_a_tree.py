'''
Calculate the diameter of a tree (not necessarily a binary tree). Diameter is the longest path between two leaves of
a tree. A path is the sum total of all distances (weights) attached to all edges between two nodes.
...
And in the diameter_of_a_tree function, you will receive string named tree = "{0,0}" and integer named ans = 0. Then,
you will need to write some code to build the tree from the input string named tree.
Note:
...
#
def diameter_of_a_tree(tree, ans):
    pass
